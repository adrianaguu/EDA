...
void OnMouseClick(int button, int state, int x, int y)
{
  ...
  {
      test.addpoint(make_pair((x-300)*1.0,(300-y)*1.0));
    //convertir x,y
...
int main(int argc, char** argv) {
   ...
   /* for (int i=0;i<4;i++)
    if (test.root->children[i]!=nullptr){
    if(test.root->children[i]->data.size()!=0)
    {
        cout<<"ind: "<< i <<":";
            for (int j=0;j<test.root->children[i]->data.size();j++)
    cout<<test.root->children[i]->data[j].first<<","<<test.root->children[i]->data[j].second<<endl;
    }
    }
    cout<<"DONE DEONE DNAEPONFPOENAOP"<<endl;
    for (int i=0;i<4;i++)
    if (test.root->children[3]->children[i]!=nullptr){
            cout<<"?";
    if(test.root->children[3]->children[i]->data.size()!=0)
    {
        cout<<"ind: "<< i <<":";
            for (int j=0;j<test.root->children[3]->children[i]->data.size();j++)
    cout<<test.root->children[3]->children[i]->data[j].first<<","<<test.root->children[3]->children[i]->data[j].second<<endl;
    }
    }*/
